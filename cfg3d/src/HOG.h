...
//    cout<<interestPointsInImage.size ()<<endl;
    ...
    for(int i=0;i<(int)interestPointsInImage.size();i++)
      {
   //     cout<<"in loop"<<endl;
...
//  mxArray *mxfeat = mxCreateNumericArray(3, out, mxDOUBLE_CLASS, mxREAL);
  ...
  for (int x = 0; x < out[1]; x++) {
    for (int y = 0; y < out[0]; y++) {
      ...
      n1 = 1.0 / sqrt(*p + *(p+1) + *(p+blocks[0]) + *(p+blocks[0]+1) + eps_hog);
      p = norm + (x+1)*blocks[0] + y;
      n2 = 1.0 / sqrt(*p + *(p+1) + *(p+blocks[0]) + *(p+blocks[0]+1) + eps_hog);
      p = norm + x*blocks[0] + y+1;
      n3 = 1.0 / sqrt(*p + *(p+1) + *(p+blocks[0]) + *(p+blocks[0]+1) + eps_hog);
      p = norm + x*blocks[0] + y;      
      n4 = 1.0 / sqrt(*p + *(p+1) + *(p+blocks[0]) + *(p+blocks[0]+1) + eps_hog);
...
public:
  ...
  void init(pcl::PointCloud<pcl::PointXYZRGBCamSL>::ConstPtr RGBDSlamFrame_)
  {
  ...
  assert(RGBDSlamFrame->size ()!=0);

  assert((int)RGBDSlamFrame->size ()== size.width*size.height); // can be 0 for dummy pcds of manually transformed
